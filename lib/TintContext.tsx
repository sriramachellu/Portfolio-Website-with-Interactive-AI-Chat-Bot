...
export function TintProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Client-side initialization: Check localStorage first
    const savedTint = localStorage.getItem('sri-portfolio-tint') as Tint | null;
    if (savedTint && ['deep-blue', 'sage', 'lavender', 'amber', 'teal', 'rose', 'clear'].includes(savedTint)) {
      setActiveTint(savedTint);
    } else {
      // Default: 75% chance for amber, 25% for others
  ...
  const setTint = useCallback((tint: Tint) => {
    setActiveTint(tint);
    localStorage.setItem('sri-portfolio-tint', tint);
